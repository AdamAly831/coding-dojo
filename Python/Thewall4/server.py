...
def new_message():
    #this is so we wont have to type in request form over and over
    post = request.form
    print post
     # test for post data
    if 'message' in post:
...
def delete_message(id):
    ...
    query = 'SELECT  * FROM messages WHERE id = :id AND user_is= :user_id'
    data = {'id': id, 'user_id': session['user_id']}
    messages - mysql.query(query,data)
